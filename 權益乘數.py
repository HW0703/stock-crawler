# -*- coding: utf-8 -*-
"""
Created on Thu May 31 20:43:30 2018
...
Shareholders_Equity = pd.read_csv('C:\\Users\\user\\Desktop\\財務\\股東權益.csv',encoding ="utf-8-sig")
Stock_Price = pd.read_csv('C:\\Users\\user\\Desktop\\財務\\年股價.csv',encoding ="utf-8-sig",index_col=0)



df_SE = Shareholders_Equity
df_SE = df_SE.drop(['代號','名稱','平均股東權益(%)'],axis = 1)

dic_EM = {}
for i in df_SE:
    for j in range (len(df_SE[i])):
        if not pd.isnull(df_SE[i][j]):
            df_SE[i][j] = eval('%.2f' %(1/(df_SE[i][j]/100)))
        else:
            df_SE[i][j] = np.nan

    dic_EM[i] = [x for x in df_SE[i] if not pd.isnull(x)]
...
for i in df_SE:
    ...
    list_75_up = []
    for j in range (len(df_SE[i])):
        if df_SE[i][j] < np.percentile(dic_EM[i], [25])[0]:
            list_25_down.append(Shareholders_Equity['代號'][j])
            dic_25_down[i] = list_25_down

        elif df_SE[i][j] >= np.percentile(dic_EM[i], [25])[0] and df_SE[i][j] < np.percentile(dic_EM[i], [50])[0]:
            list_25_50_between.append(Shareholders_Equity['代號'][j])
            dic_25_50_between[i] = list_25_50_between

        elif df_SE[i][j] >= np.percentile(dic_EM[i], [50])[0] and df_SE[i][j] < np.percentile(dic_EM[i], [75])[0]:
            list_50_75_between.append(Shareholders_Equity['代號'][j])
            dic_50_75_between[i] = list_50_75_between

        elif df_SE[i][j] > np.percentile(dic_EM[i], [75])[0]:
            list_75_up.append(Shareholders_Equity['代號'][j])
