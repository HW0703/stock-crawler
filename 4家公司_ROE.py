# -*- coding: utf-8 -*-
"""
Created on Wed May 30 16:11:43 2018
...
Return_On_Equity = pd.read_csv('C:\\Users\\user\\Desktop\\財務\\ROE.csv',encoding ="utf-8-sig")
Stock_Price = pd.read_csv('C:\\Users\\user\\Desktop\\財務\\年股價.csv',encoding ="utf-8-sig",index_col=0)

df_ROE = Return_On_Equity
df_ROE = df_ROE.drop(['代號','名稱','平均ROE(%)'],axis = 1)

dic_ROE = {}
for i in df_ROE:
    dic_ROE[i] = [x for x in df_ROE[i] if not pd.isnull(x)]
...
for i in df_ROE:
    ...
    list_50_75_between = []
    list_75_up = []
    for j in range (len(df_ROE[i])):
        if df_ROE[i][j] < np.percentile(dic_ROE[i], [25])[0]:
            list_25_down.append(Return_On_Equity['代號'][j])
            dic_25_down[i] = list_25_down

        elif df_ROE[i][j] >= np.percentile(dic_ROE[i], [25])[0] and df_ROE[i][j] < np.percentile(dic_ROE[i], [50])[0]:
            list_25_50_between.append(Return_On_Equity['代號'][j])
            dic_25_50_between[i] = list_25_50_between

        elif df_ROE[i][j] >= np.percentile(dic_ROE[i], [50])[0] and df_ROE[i][j] < np.percentile(dic_ROE[i], [75])[0]:
            list_50_75_between.append(Return_On_Equity['代號'][j])
            dic_50_75_between[i] = list_50_75_between

        elif df_ROE[i][j] > np.percentile(dic_ROE[i], [75])[0]:
            list_75_up.append(Return_On_Equity['代號'][j])
